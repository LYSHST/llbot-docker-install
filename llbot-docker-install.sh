    ...
    while :; do
        ...
        case $ob11_type in
            ...
            1)
                read -p "端口: " port
                token=""
                while [ -z "$token" ]; do read -p "Token (必填): " token; done
            ...
            3)
                ...
                while [ -z "$token" ]; do read -p "Token (必填): " token; done
                SERVICE_PORTS["$port"]=1
                OB11_CONNECTS+=("{\"type\":\"http\",\"enable\":true,\"host\":\"\",\"port\":$port,\"token\":\"${token}\",\"reportSelfMessage\":false,\"reportOfflineMessage\":false,\"messageFormat\":\"array\",\"debug\":false,\"heartInterval\":30000}")
                ;;
            4)
...
if [[ "$protocol_choices" =~ 2 ]]; then
    ...
    while [ -z "$MILKY_HTTP_TOKEN" ]; do read -p "Access Token (必填): " MILKY_HTTP_TOKEN; done
    echo ""
...
if [[ "$protocol_choices" =~ 3 ]]; then
    ...
    while [ -z "$SATORI_TOKEN" ]; do read -p "Token (必填): " SATORI_TOKEN; done
fi
...
if [[ "$use_docker_mirror" =~ ^[yY]$ ]]; then
  DOCKER_MIRRORS=("docker.gh-proxy.cn")
  test_mirror() {
    local mirror=$1
    local image=$2
    local tag=$3
    echo "测试镜像源 ${mirror} 的 ${image}:${tag} ..." >&2
    if timeout 30 docker manifest inspect "${mirror}/linyuchen/${image}:${tag}" > /dev/null 2>&1; then return 0; fi
    return 1
  }
...
# ====================== 后置修改配置（官方流程全部跑完后执行） ======================
echo -e "\n==================== 后置修改配置 ===================="
echo "1. 替换镜像源为 docker.1ms.run"
echo "2. 端口改为 [::] 同时监听 IPv4/IPv6"
echo "3. 升级为双容器 pmhq + llbot 架构"

# 读取变量
MIRROR_PREFIX="docker.1ms.run/"
PMHQ_IMG="linyuchen/pmhq:latest"
LLBOT_IMG="linyuchen/llbot:latest"

# 重建全新双容器compose，覆盖原文件
cat > docker-compose.yml <<YAML
version: "3.8"
services:
  pmhq:
    image: ${MIRROR_PREFIX}${PMHQ_IMG}
    privileged: true
    environment:
...
echo "✅ 后置修改完成："
echo " - 镜像源固定 docker.1ms.run"
echo " - 双容器 pmhq + llbot latest"
echo " - 端口 [::]:${WEBUI_PORT} 兼容 IPv4 / IPv6"
echo ""
echo "如需应用新配置执行："
echo "sudo docker compose down && sudo docker compose pull && sudo docker compose up -d"
